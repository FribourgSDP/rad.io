...
class MusixmatchLyricsGetterTest {
    private val searchedSong1Name = "hurricane"
    private val searchedSong1Artist = "bob dylan"
    private val searchedSong1Lyrics = "Pistol shots ring out in the barroom night"
    private val lyricslessSongName = "stream"
    ...
    fun getLyricsFromSongAndArtist(){
        val f = MusixmatchLyricsGetter.getLyrics(searchedSong1Name, searchedSong1Artist, MockOkHttpClient())
        val lyrics = f.get()
        assertTrue("actual : $lyrics",lyrics.startsWith(searchedSong1Lyrics))
    }
    @Test
    fun emptyLyricsTest(){
        val lyricsFuture = MusixmatchLyricsGetter.getLyrics(lyricslessSongName, lyricslessArtist, MockOkHttpClient())
        checkLyricsNotFound(lyricsFuture)
    }
    @Test(expected = Exception::class)
    fun songIDSongNotFoundTest(){
        val songIDFuture = MusixmatchLyricsGetter.getSongID(unknownSongName, unknownArtistName, MockOkHttpClient())
        songIDFuture.get()
    }
    @Test
    fun songNotFoundTest(){
        val lyricsFuture = MusixmatchLyricsGetter.getLyrics(unknownSongName, unknownArtistName, MockOkHttpClient())
        checkLyricsNotFound(lyricsFuture)
    }
    @Test
    ...
    class MockOkHttpClient : OkHttpClient(){
        ...
        class MockCall(private val req: Request) : Call{
            ...
            override fun enqueue(rc: Callback) {
                val s = when {
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.search?q_track=hurricane&q_artist=bob%20dylan&apikey=a3454edb65483e706c127deaa11df69d&s_artist_rating=desc, tag=null}" -> {
                    ...
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=115119412&apikey=a3454edb65483e706c127deaa11df69d, tag=null}" -> {
                    ...
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.search?q_track=stream&q_artist=dream%20theater&apikey=a3454edb65483e706c127deaa11df69d&s_artist_rating=desc, tag=null}" -> {
                    ...
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=37042055&apikey=a3454edb65483e706c127deaa11df69d, tag=null}" -> {
                        "{\"message\":{\"header\":{\"status_code\":404,\"execute_time\":0.011165142059326},\"body\":[]}}"
                    }
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.search?q_track=fsdgfdgdfgdfgdfg&q_artist=weoir%20hpfasdsfno&apikey=a3454edb65483e706c127deaa11df69d&s_artist_rating=desc, tag=null}" -> {
                        "{\"message\":{\"header\":{\"status_code\":200,\"execute_time\":0.01686692237854,\"available\":0},\"body\":{\"track_list\":[]}}}"
                    }
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.search?q_track=rouge&q_artist=sardou&apikey=a3454edb65483e706c127deaa11df69d&s_artist_rating=desc, tag=null}" -> {
                    ...
                    req.toString() == "Request{method=GET, url=http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=39648585&apikey=a3454edb65483e706c127deaa11df69d, tag=null}" -> {
